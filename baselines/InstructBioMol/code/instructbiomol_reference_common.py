...
def build_instruction(task_type: str, sample: Dict, input_mode: str) -> str:
    ...
    if task_type == "class":
        return (
            f"{context}What is the category name of this {task_name} fragment? "
            "Answer with the category name or a short sentence."
        )
    return (
        f"{context}Please provide a functional description of this {task_name} "
        "fragment, including likely function, important residues, mechanism, binding, "
        "or conservation context when inferable."
    )
