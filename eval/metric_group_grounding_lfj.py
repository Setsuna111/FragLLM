...
argParser.add_argument("--results_path", default='./eval_results/grounding_group/grounding_lora_0529_all_123500/ActGroundGroup_results.csv', type=str, help="path to save the generated description")
argParser.add_argument("--iou_threshold", type=float, default=0.5, help="iou threshold")
argParser.add_argument("--bert_threshold", default=1.0, type=float, help="bert threshold")
argParser.add_argument("--model_path", type=str, default="/home/dataset-local/projects_dir/pretrained_model/biobert-large-cased-v1.1/", help="path to the prot2text model")
argParser.add_argument("--embedding_path", type=str, default="/home/dataset-local/projects_dir/FragLLM/eval/cache/VenusX_Act.npz", help="path to the embedding file")
argParser.add_argument("--data_root", type=str, default="/home/dataset-local/projects_dir/FragLLM/data_70", help="root directory containing VenusX dataset splits")
argParser.add_argument("--batch_size", type=int, default=32, help="batch size for precomputing category embeddings")
...
def get_bert_embeddings(texts, batch_size):
    ...
    for start in range(0, len(texts), batch_size):
        ...
        print(f'Processed batch {start//batch_size + 1}/{(len(texts) + batch_size - 1) // batch_size}')
    return np.vstack(embeddings)


def infer_dataset_name_from_embedding_path(embedding_path):
    stem = os.path.splitext(os.path.basename(embedding_path))[0]
    return stem if stem.startswith("VenusX_") else f"VenusX_{stem}"


def load_dataset_category_names(data_root, dataset_name):
    category_names = []
    seen = set()
    source_files = [
        os.path.join(data_root, dataset_name, "train.json"),
        os.path.join(data_root, dataset_name, "test.json"),
    ]
    ...
    if not category_names:
        raise ValueError(f"No category labels found in {os.path.join(data_root, dataset_name)}")
    return category_names, source_files


def category_embedding_cache_is_valid(embedding_path, dataset_name):
    if not os.path.exists(embedding_path):
        return False
    try:
        data = np.load(embedding_path, allow_pickle=True)
        return (
            "feature_vectors" in data.files
            and "category_names" in data.files
            and "dataset_name" in data.files
            and str(data["dataset_name"]) == dataset_name
        )
    except Exception:
        return False


def ensure_category_embedding_cache(embedding_path, data_root, batch_size):
    dataset_name = infer_dataset_name_from_embedding_path(embedding_path)
    if category_embedding_cache_is_valid(embedding_path, dataset_name):
        return

    category_names, source_files = load_dataset_category_names(data_root, dataset_name)
    if os.path.exists(embedding_path):
        print(f"Rebuilding category embedding cache for {dataset_name}: {embedding_path}")
    else:
        print(f"Precomputing {len(category_names)} category embeddings for {dataset_name}: {embedding_path}")

    feature_vectors = get_bert_embeddings(category_names, batch_size)
    ...
    print(f"Saved category embedding cache: {embedding_path}")
...
# extract the target from the responses FIXME:需要根据响应格式调整
def extract_target(response):
    # 找到response中第一个“:”的位置
...
def evaluate_group_grounding(args: Dict[str, Any]) -> Dict[str, Any]:
    ensure_category_embedding_cache(args.embedding_path, args.data_root, args.batch_size)
    data_npz = np.load(args.embedding_path, allow_pickle=True)
