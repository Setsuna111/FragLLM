...
from instructbiomol_reference_common import (  # noqa: E402
    FoldseekRunner,
    add_common_args,
    batched,
    full_structure_path,
...
def build_single_prompt(sample: Dict) -> str:
    return (
        f"Locate all regions of the protein that correspond to the {sample['task_name']} "
        f"category \"{sample['category']}\". "
        f"The protein sequence has length {len(sample['sequence'])}. "
        "Use 0-based inclusive residue coordinates. "
        "Return only one machine-readable answer in this exact form: "
        '<loc>{"regions":[[start,end],...]}</loc>. '
        "Do not explain the protein function. Do not include any text outside the <loc> tag. "
        "If no region is found, return <loc>{\"regions\":[]}</loc>."
    )


def build_single_reference(sample: Dict) -> str:
    return f"<loc>{{\"regions\":{format_regions(sample['regions'])}}}</loc>"
...
def evaluate_dataset(dataset_name: str, model, args: argparse.Namespace) -> Path:
    ...
    if args.dry_run:
        ...
        out_dir = make_output_dir(dry_args, "grounding_single")
    else:
        out_dir = make_output_dir(args, "grounding_single")
