...
from dataset.data_catalog_moledit import MyConcatMolEditProtDataset, DatasetMolEditCatalog, JSONDataset4MolEdit
...
def parser_args():
    ...
    parser.add_argument('--exp_name', default='Mol-Edit', type=str)
...
class Runner:
    def __init__(self, args):
        self.args = args
        self.init_ds()
        self.model = UniModel4MolEdit(args)
    ...
    def load_train_dataset(self):
        ...
        if self.args['local_rank'] > 0:
            torch.distributed.barrier()
        concat_data = MyConcatMolEditProtDataset(self.args['dataset_name_list'])
        if self.args['local_rank'] == 0:
            torch.distributed.barrier()
    ...
    def load_val_dataset(self):
        ...
        for dataset_name in self.args['val_dataset_name_list']:
            dataset_dict = getattr(DatasetMolEditCatalog(), dataset_name)
