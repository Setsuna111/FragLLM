...
def add_common_args(parser: argparse.ArgumentParser, default_datasets: str) -> None:
    ...
    parser.add_argument("--model_identifier", default="instructbiomol_instruct_ref_v6")
...
def build_instruction(task_type: str, sample: Dict, input_mode: str) -> str:
    ...
    if input_mode == "full":
        ...
        context = f"The target fragment sequence is {fragment}. "
        if task_type == "class":
            return (
                f"{context}What is the category name of the {task_name} represented by "
                "this target fragment? Answer with the category name or a short sentence."
            )
        return (
            f"{context}Please describe the function of the {task_name} represented by "
            "this target fragment, including important residues, mechanism, binding, "
            "or conservation context when inferable."
        )

    if task_type == "class":
        return (
            f"What is the category name of this {task_name} fragment? "
            "Answer with the category name or a short sentence."
        )
    return (
        f"Please provide a functional description of this {task_name} fragment, "
        "including likely function, important residues, mechanism, binding, or "
        "conservation context when inferable."
    )
