...
class SelfAttention(nn.Module):
    """
    Self-attention mechanism for protein sequence encoding.
    Non-causal attention where all tokens can see each other.
    """
    ...
    def forward(
        ...
        # Project to Q, K, V
        Q = self.query_proj(hidden_states)  # (batch_size, seq_len, hidden_size)
        K = self.key_proj(hidden_states)    # (batch_size, seq_len, hidden_size)
        V = self.value_proj(hidden_states)  # (batch_size, seq_len, hidden_size)
        ...
        # Apply attention mask if provided (non-causal, all tokens can see each other)
...
class PositionDecoder(nn.Module):
    """
    Enhanced position decoder for predicting start and end positions of functional regions.
    Each layer contains: self-attention → residual → cross-attention → residual → feedforward → residual
    """
    ...
    def __init__(
        ...
        # If num_self_attention_heads not specified, use same as cross-attention
        if num_self_attention_heads is None:
            num_self_attention_heads = num_attention_heads
        
        # Self-attention layers for enhanced sequence modeling
        self.self_attention_layers = nn.ModuleList([
            SelfAttention(hidden_size, num_self_attention_heads, dropout_rate)
        ...
        # Layer norms - need 3 per layer now (self-attn, cross-attn, ff)
        self.layer_norms_self = nn.ModuleList([
            nn.LayerNorm(hidden_size) for _ in range(num_layers)
        ])
        self.layer_norms_cross = nn.ModuleList([
            nn.LayerNorm(hidden_size) for _ in range(num_layers)
        ])
        ...
        # Position prediction heads
        self.position_head = nn.Sequential(
            nn.Linear(hidden_size, intermediate_size),
            nn.ReLU(),
            nn.Dropout(dropout_rate),
            nn.Linear(intermediate_size, 2)  # 2 for start and end positions
        )
    ...
    def forward(
        ...
        Returns:
            Position logits for each sequence position (start/end probabilities)
        """
        # Use protein embeddings as initial hidden states
        hidden_states = protein_embeddings
        
        # Apply transformer layers with enhanced architecture
        for i in range(self.num_layers):
            # 1. Self-attention: all sequence tokens can see each other (non-causal)
            # todo: 1016 test, remove self-attention to recover performance
            residual = hidden_states
            self_attended = self.self_attention_layers[i](
                hidden_states=hidden_states,
                attention_mask=protein_attention_mask
            )
            hidden_states = self.layer_norms_self[i](residual + self_attended)
            
            # 2. Cross-attention: protein sequence attends to prompt
            residual = hidden_states
            cross_attended = self.cross_attention_layers[i](
                query=hidden_states,
                key=prompt_embeddings,
                value=prompt_embeddings
            )
            hidden_states = self.layer_norms_cross[i](residual + cross_attended)
        ...
        # Predict positions
        position_logits = self.position_head(hidden_states)  # (batch_size, seq_len, 2)
        ...
        if protein_attention_mask is not None:
            ...
            position_logits = position_logits.masked_fill(mask == 0, -1e9)
    ...
    def predict_positions(
        ...
        """
        Predict start and end positions for functional regions.
        ...
        Returns:
            Predicted start positions, end positions, and optionally probabilities
        """
        position_logits = self.forward(
            protein_embeddings, 
            prompt_embeddings, 
        ...
        # Get predicted positions (argmax)
        start_positions = torch.argmax(start_logits, dim=1)  # (batch_size,)
        end_positions = torch.argmax(end_logits, dim=1)      # (batch_size,)
        
        probabilities = None
        if return_probabilities:
            start_probs = F.softmax(start_logits, dim=1)
            end_probs = F.softmax(end_logits, dim=1)
            probabilities = torch.stack([start_probs, end_probs], dim=-1)  # (batch_size, seq_len, 2)
        
        return start_positions, end_positions, probabilities