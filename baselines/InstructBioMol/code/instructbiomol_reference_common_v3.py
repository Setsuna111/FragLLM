...
def add_common_args(parser: argparse.ArgumentParser, default_datasets: str) -> None:
    ...
    parser.add_argument("--model_identifier", default="instructbiomol_instruct_prompt_v3")
...
def load_reference_samples(
    ...
    candidate_categories = load_train_category_candidates(data_root, source_name)
...
def load_train_category_candidates(data_root: Path, source_name: str) -> List[str]:
    train_path = data_root / source_name / "train.json"
    candidates: List[str] = []
    ...
    with train_path.open("r", encoding="utf-8") as handle:
        for item in json.load(handle):
            for fragment_group in item.get("fragments", []):
                category = fragment_group.get("category")
...
def build_instruction(task_type: str, sample: Dict, input_mode: str) -> str:
    ...
    candidate_categories = sample.get("candidate_categories", [])
    if input_mode == "full":
        ...
        if task_type == "class":
            ...
            if candidate_categories:
                candidates_text = (
                    "Choose one answer from these training-set category candidates when possible: "
                    + " | ".join(candidate_categories)
                    + ". "
                )
            return (
                f"{context}Identify the specific database-style category name of "
                f"the {task_name} represented by this target fragment. {candidates_text}Return only "
        ...
        return (
            f"{context}Write the database entry description for the {task_name} "
            "represented by this exact target fragment. Do not summarize the whole "
            "protein. If uncertain, describe the most specific site/domain/motif "
            "type supported by the fragment sequence. Avoid inventing residue "
            "numbers, coordinates, or unsupported mechanisms."
        )
    ...
    return (
        f"{context}Write the database entry description for this {task_name} "
        "fragment. Do not summarize the whole protein. If uncertain, describe "
        "the most specific site/domain/motif type supported by the fragment "
        "sequence. Avoid inventing residue numbers, coordinates, or unsupported mechanisms."
    )
