...
Loads model and runs inference on specified test datasets with ESM cache and llama cache support.

This script supports:
1. Pre-computed ESM embeddings for faster inference
2. Category embeddings cache
3. Multiple dataset inference
"""
...
def run_dataset_inference(
    ...
    Returns:
        Tuple of (all_results, all_metrics)
    """
    model.eval()
    all_results = []
    all_metrics = []
...
def main():
    ...
    if use_esm_cache:
        ...
        datasets, collator = get_datasets_and_collator_with_esm_cache(
            ...
            null_position_prob=0.0,  # No null positions during inference
            random_position_prob=0.0,  # No random positions during inference
            position_noise_std=0.0  # No position noise during inference
        )
    else:
        # Use internal ESM encoder
    ...
    results, metrics = run_dataset_inference(
        model,
        test_loader,
        args.device,
        use_external_esm=use_esm_cache
    )
