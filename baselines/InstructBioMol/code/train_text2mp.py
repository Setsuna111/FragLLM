...
from dataset.data_catalog_text2molprot import MyConcatText2MolProtDataset, DatasetText2MolProtCatalog, JSONDataset4Text2MolProt
from dataset.sampler import DistributedBatchSampler, DistributedWeightedAccumulateMultiDatasetBatchSampler
...
# from dataset.collator4t5 import DataCollatorForT5MLM
...
def parser_args():
    ...
    parser.add_argument('--add_token', action='store_true')
...
class Runner:
    def __init__(self, args):
        self.args = args
        self.init_ds()
        self.model = UniModel4Text2ProtMol(args)

        self.train_dataset, self.iter_, _ = self.load_train_dataset()
        self.val_datasets, self.val_loaders = self.load_val_dataset()
    ...
    def load_train_dataset(self):
        ...
        if self.args['local_rank'] > 0:
            torch.distributed.barrier()
        concat_data = MyConcatText2MolProtDataset(self.args['dataset_name_list'])
        if self.args['local_rank'] == 0:
            torch.distributed.barrier()
    ...
    def load_val_dataset(self):
        val_dataset_list = []
        val_loader_list = []
        for dataset_name in self.args['val_dataset_name_list']:
            dataset_dict = getattr(DatasetText2MolProtCatalog(), dataset_name)
            ...
            val_loader_list.append(loader)
        return val_dataset_list, val_loader_list
