...
def add_common_args(parser: argparse.ArgumentParser, default_datasets: str) -> None:
    ...
    parser.add_argument("--model_identifier", default="instructbiomol_instruct_prompt_v4")
...
def build_instruction(task_type: str, sample: Dict, input_mode: str) -> str:
    ...
    if input_mode == "full":
        ...
        if task_type == "class":
            return (
                ...
                f"the {task_name} represented by this target fragment. Fill this "
                "template with the most specific site label: '<family or protein>, active site'. "
                "Return only the filled label. Do not output an enzyme activity, "
                "biological process, sentence, coordinates, or full-protein family "
                "unless it is part of the site label."
            )
        return (
            f"{context}Complete one InterPro-style entry paragraph for this exact "
            f"{task_name}. Start with 'This entry represents'. Describe the target "
            "site/domain/motif type and its conserved function, not the whole protein. "
            "Do not include coordinates or unsupported residue numbers."
        )
    ...
    if task_type == "class":
        return (
            ...
            f"{task_name} fragment. Fill this template with the most specific site "
            "label: '<family or protein>, active site'. Return only the filled label."
        )
    return (
        f"{context}Complete one InterPro-style entry paragraph for this exact "
        f"{task_name}. Start with 'This entry represents'. Describe the target "
