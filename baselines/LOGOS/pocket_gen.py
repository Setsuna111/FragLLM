...
This script reads protein pocket data (JSONL), generates protein sequences
conditioned on input prompts using the language model, and saves generated
...
TRANS_TAG = "<Trans>"
PROTEINS_TAG = "<ProteinS>"
PROTEINE_TAG = "<ProteinE>"

PROTEIN_PATTERN = re.compile(r"<ProteinS>(.*?)<ProteinE>", re.DOTALL)


def extract_result(section: str) -> List[str]:
    """Extract protein-pocket strings enclosed by <ProteinS> ... <ProteinE>."""
    return [match.strip() for match in PROTEIN_PATTERN.findall(section) if match.strip()]
...
def evaluate_sampleN(
    ...
    for data in inputs:
        prompts.append(data["text"] + PROTEINS_TAG)
        gts.extend([data["id"]] * num_samples)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Pocket generation.")
    parser.add_argument(
        "--data_file",
        type=str,
        required=True,
        help="Path to pocket JSONL dataset.",
    )
    parser.add_argument(
        "--model_path",
        type=str,
...
def main() -> None:
    ...
    evaluator = LMEvaluator(
        ...
        task_name="pocket_trans",
        data_manager="pocket_trans",
        device=accelerator.device,
    )

    output_path = os.path.join(
        args.results_dir,
        f"pocket_gen_{args.temperature}_{args.top_p}_{args.repetition_penalty}",
        data_name,
    )
    ...
    with open(output_file, "w", encoding="utf-8") as results_file:
        for i, batch in enumerate(
            ...
            for seq, id_, ppl in zip(res["generated_raw"], res["ids"], res["ppls"]):
                print(seq, id_, ppl)
                line = json.dumps(
                    {"sequence": seq, "groundtruth": id_, "ppl": ppl},
                    ensure_ascii=False,
                    separators=(",", ":"),
                )
