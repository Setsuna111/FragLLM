...
class FragmentPositionDecoder(nn.Module):
    ...
    def forward(self, latents: Tensor, hidden_states: Tensor) -> Tensor:
        ...
        latents = self.ffn(residuals + latents) + residuals
        out: Tensor = self.output_layer_norm(latents)
        out = self.output_proj(out) # (L, 1022)
        return out
...
class ProteinMetaModel:
    def __init__(self, config):
        ...
        if hasattr(config, "esm_path"):
            ...
            self.fragment_position_decoder = FragmentPositionDecoder01(config.hidden_size, 1, config.num_perceiver_heads, config.dropout_rate)
    ...
    def initialize_modules(self, model_args, fsdp=None):
        ...
        if getattr(self, "fragment_position_decoder", None) is None:
            self.fragment_position_decoder = FragmentPositionDecoder01(self.config.hidden_size, 1, self.config.num_perceiver_heads, self.config.dropout_rate)
