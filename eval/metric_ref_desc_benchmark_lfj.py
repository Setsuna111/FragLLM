...
def compute_bert_score(predictions: List[str], references: List[str]) -> Dict[str, Dict[str, Any]]: 
    ...
    roberta_results = bert.compute(predictions=truncated_predictions, references=truncated_labels, model_type="/home/dataset-local/projects_dir/pretrained_model/roberta_large", num_layers=17)
...
def get_nested_metric(results: Dict[str, Any], path: Tuple[str, ...]) -> Any:
    metric_value: Any = results
    for key in path:
        if not isinstance(metric_value, dict) or key not in metric_value:
            return None
        metric_value = metric_value[key]
    return metric_value


def format_metric_value(metric_value: Any) -> str:
    try:
        return f"{float(metric_value):.4g}"
...
if __name__ == "__main__": 
    ...
    results = benchmark_csv(parsed_args.__dict__)
    print_plot_metrics(results)
